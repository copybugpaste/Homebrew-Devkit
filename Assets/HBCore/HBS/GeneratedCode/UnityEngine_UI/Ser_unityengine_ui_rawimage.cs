...
using System;
namespace HBS {
    public static class Ser_unityengine_ui_rawimage {
        public static void Ser( HBS.Writer writer , object oo ) {
            if( writer.WriteNull(oo)) { return; }
            HBS.Writer writer_ASXDRGBHU;
            UnityEngine.UI.RawImage o = (UnityEngine.UI.RawImage)oo;
            writer.Write(10);

            writer.Write("texture");
            writer_ASXDRGBHU = new HBS.Writer();
            HBS.Ser_unityengine_texture.Ser( writer_ASXDRGBHU , o.texture); //property
            writer.Write(writer_ASXDRGBHU.stream.ToArray());
            writer_ASXDRGBHU.Close();

            writer.Write("uvRect");
            writer_ASXDRGBHU = new HBS.Writer();
            HBS.Ser_unityengine_rect.Ser( writer_ASXDRGBHU , o.uvRect); //property
            writer.Write(writer_ASXDRGBHU.stream.ToArray());
            writer_ASXDRGBHU.Close();
            ...
            for (int i_ASXDRGBHU = 0; i_ASXDRGBHU < count_ASXDRGBHU; i_ASXDRGBHU++) {
                ...
                if (name_ASXDRGBHU == "texture") {
                    try {
                        ...
                        o.texture = (UnityEngine.Texture)HBS.Ser_unityengine_texture.Res( reader_ASXDRGBHU ); //property
                        reader_ASXDRGBHU.Close();
                    } catch { }
                }

                if (name_ASXDRGBHU == "uvRect") {
                    try {
                        reader_ASXDRGBHU = new HBS.Reader(data_ASXDRGBHU);
                        o.uvRect = (UnityEngine.Rect)HBS.Ser_unityengine_rect.Res( reader_ASXDRGBHU ); //property
                        reader_ASXDRGBHU.Close();
                    } catch { }
                }
