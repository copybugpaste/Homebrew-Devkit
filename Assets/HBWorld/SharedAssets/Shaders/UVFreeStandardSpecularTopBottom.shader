...
Shader "UVFree/StandardSpecular/Top-Bottom-Texture" {
	Properties {
		...
		_EmissionMap("DetailTexture", 2D) = "white" {}
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
			...
			//
			...
			#ifdef _EMISSION
				half3 emissionX = tex2D(_EmissionMap, xUV*10).rgb * _EmissionColor.rgb;
				half3 emissionY = tex2D(_EmissionMap, yUV*10).rgb * _EmissionColor.rgb;
				
				half3 emissionYTop = tex2D(_TopEmissionMap, yUVTop*10).rgb * _TopEmissionColor.rgb;
				...
				half3 emissionZ = tex2D(_EmissionMap, zUV*10).rgb * _EmissionColor.rgb;
