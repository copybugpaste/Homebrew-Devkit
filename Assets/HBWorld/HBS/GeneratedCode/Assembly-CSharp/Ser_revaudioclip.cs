...
namespace HBS {
    public static class Ser_revaudioclip {
        public static void Ser( HBS.Writer writer , object oo ) {
            ...
            writer_ASXDRGBHU = new HBS.Writer();
            if( writer_ASXDRGBHU.WriteNull(o.samples) == false ) {
                writer_ASXDRGBHU.Write(o.samples.Length);
                for( int i = 0; i < o.samples.Length; i++ ) {
                    writer_ASXDRGBHU.Write(o.samples[i]); //field primitive
                }
            }
            writer.Write(writer_ASXDRGBHU.stream.ToArray());
            writer_ASXDRGBHU.Close();
        ...
        public static object Res( HBS.Reader reader, object oo = null) {
            ...
            for (int i_ASXDRGBHU = 0; i_ASXDRGBHU < count_ASXDRGBHU; i_ASXDRGBHU++) {
                ...
                if (name_ASXDRGBHU == "samples") {
                    try {
                        reader_ASXDRGBHU = new HBS.Reader(data_ASXDRGBHU);
                    if( reader_ASXDRGBHU.ReadNull() == false ) {
                        System.Single[] samples_arr = new System.Single[(int)reader_ASXDRGBHU.Read()];
                        for( int i = 0; i < samples_arr.Length; i++ ) {
                                samples_arr[i] = (System.Single)reader_ASXDRGBHU.Read(); //field primitive
                            }
                            o.samples = samples_arr;
                         }
                        reader_ASXDRGBHU.Close();
                    } catch { }
                }
