...
public class ZipUtil {
    ...
    public static void Unzip(string zipFilePath, string location, ExtractExistingFileAction action = ExtractExistingFileAction.OverwriteSilently) {
#if UNITY_EDITOR || UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_STANDALONE_LINUX
        Directory.CreateDirectory(location);

        using (ZipFile zip = ZipFile.Read(zipFilePath)) {

            zip.ExtractAll(location, action);
        }
#elif UNITY_ANDROID
    ...
    public static void Zip(string ZipFileName, string Folder) {
